...
| 2026-05-28 | v2.1 | **剪贴板增强**：支持 CF_HDROP 原生文件格式；**失败自动跳过**；**"未命名"过滤**；**scan_to_clipboard**；**等待优化**：PDF 间隔从 8 秒降至 1 秒 |
| 2026-05-28 | v2.2 | **批量上传模式**：去掉逐个轮询，快速上传全部 PDF 后统一检测；**页面自动滚动**；**上传记录**：批量写入 markdown；**精确绑定**：`md_mapping.json`；**超时优化**：600 秒 → 120 秒 |
| 2026-05-29 | v2.3 | **编码修复**：Windows GBK 终端 UTF-8 重包装，支持 emoji 文件名；**记录格式重构**：绑定状态统一在批量上传块内标记，不再生成分散小块 |
